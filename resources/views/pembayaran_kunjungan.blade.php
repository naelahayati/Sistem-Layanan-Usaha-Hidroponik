...
@section('konten')
<link rel="stylesheet" href="{{ asset('css/pengguna/pembayaran_kunjungan.css') }}">
<div class="payment-wrapper pembayaran-kunjungan-page">
        ...
        <div class="payment-flex-container">
            ...
            <div class="payment-right-col">
                <div class="amount-box mb-4" style="text-align: left; background: #ebf5eb; border: 1px solid #c2e0c2;">
                    ...
                    <div class="amount-value" style="color: #1b3a1a; font-size: 2rem;">Rp {{ number_format($reservasi->total_harga, 0, ',', '.') }}</div>
                </div>
                <div class="integrated-summary">
                    <h6 class="instructions-title" style="margin-top: 0; margin-bottom: 15px;">Detail Reservasi:</h6>
                    <div class="summary-row">
                        <span>ID Reservasi</span>
                        <span>#{{ str_pad($reservasi->id_reservasi, 5, '0', STR_PAD_LEFT) }}</span>
                    </div>
                    <div class="summary-row">
                        <span>Paket</span>
                        <span>{{ $reservasi->paket_name }}</span>
                    </div>
                    <div class="summary-row">
                        <span>Tanggal</span>
                        <span>{{ \Carbon\Carbon::parse($reservasi->tanggal_reservasi)->translatedFormat('d F Y') }}</span>
                    </div>
                    <div class="summary-row">
                        <span>Peserta</span>
                        <span>{{ $reservasi->jumlah_peserta }} Orang</span>
                    </div>
                </div>
...
<script>
    ...
    document.getElementById('formBuktiPembayaran').addEventListener('submit', function(e) {
        ...
        fetch("{{ route('nazfram.pembayaran_kunjungan.konfirmasi', $reservasi->id_reservasi) }}", {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .then(data => {
            if (data.success) {
                Swal.fire('Berhasil!', data.message, 'success').then(() => {
                    window.location.href = "{{ route('reservasi.riwayat') }}?payment_success=true";
                });
            } else {
    ...
    // Timer Logic
    const expiresAt = new Date("{{ \Carbon\Carbon::parse($reservasi->expires_at)->toIso8601String() }}").getTime();
    ...
    function handleExpiration() {
        Swal.fire({
            ...
            window.location.href = "{{ route('nazfram.kunjungan') }}";
        });
    }
    ...
    document.getElementById('btn-batalkan-transaksi').onclick = function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch("{{ route('nazfram.pembayaran_kunjungan.batal', $reservasi->id_reservasi) }}", {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire('Dibatalkan', 'Transaksi reservasi Anda telah dibatalkan.', 'success')
                        .then(() => window.location.href = "{{ route('nazfram.kunjungan') }}");
                    }
                });
            }
        });
