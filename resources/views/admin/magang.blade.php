            ...
            <div class="col-md-3 col-sm-4 col-6 mb-4">
                <div class="card h-100 position-relative shadow-sm viewMagangCard" style="cursor:pointer; border-radius: 10px; overflow: hidden;" data-magang="{{ json_encode($magang) }}">
                    ...
                    <div class="position-absolute" style="top: 10px; left: 10px; z-index: 10;">
                        <span class="badge {{ $magang->is_active ? 'badge-success' : 'badge-danger' }} p-2">
                            {{ $magang->is_active ? 'Aktif' : 'Non-aktif' }}
                        </span>
                    </div>
                    ...
                    <div class="position-absolute" style="bottom: 10px; right: 10px; z-index: 15;">
                        <button type="button" class="btn btn-sm toggleStatusBtn {{ $magang->is_active ? 'btn-success' : 'btn-secondary' }}" data-id="{{ $magang->id }}" title="{{ $magang->is_active ? 'Matikan Paket' : 'Aktifkan Paket' }}">
                            <i class="fas {{ $magang->is_active ? 'fa-toggle-on' : 'fa-toggle-off' }}"></i>
                        </button>
                        <a href="{{ route('admin.magang.edit_form', $magang->id) }}" class="btn btn-sm btn-light text-primary" style="margin-left: 4px;">
                            <i class="fas fa-edit"></i>
                        </a>
                        <button type="button" class="btn btn-sm btn-danger deleteMagangBtn" data-id="{{ $magang->id }}" style="margin-left: 4px;">
